                    ...
                    <form action="{{ route('pengajar.materi.store') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-5">
                            ...
                            <textarea id="deskripsi" name="deskripsi" rows="4"
                                ...
                                focus:border-indigo-500 focus:ring focus:ring-indigo-200 dark:focus:ring-indigo-600 rounded-md shadow-sm"
                                placeholder="Tuliskan ringkasan atau penjelasan singkat materi di sini...">{{ old('deskripsi') }}</textarea>
                            <x-input-error :messages="$errors->get('deskripsi')" class="mt-2" />
                        </div>

                        <!-- Link Video YouTube -->
                        <div class="mb-5">
                            <x-input-label for="link_video" value="🎥 Link Video YouTube (Opsional)" />
                            <x-text-input id="link_video" name="link_video" type="text" class="mt-1 block w-full"
                                placeholder="Contoh: https://youtu.be/4VbyF6Afma0" :value="old('link_video')" />
                            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                Salin-tempel link YouTube dari browser, misal dari channel pribadi atau referensi.
                            </p>
